...
public class OctopusForm extends WaterAbility {
	...
	public OctopusForm(final Player player) {
		...
		this.interval = applyInverseModifiers(getConfigSection().getLong("FormDelay"));
		this.attackRange = applyModifiers(getConfigSection().getInt("AttackRange")); // ------------->    //Although this benefits from being smaller, it's better
		this.usageCooldown = applyInverseModifiers(getConfigSection().getInt("UsageCooldown")); 	//to scale it up with the radius as well
		...
		this.duration = applyModifiers(getConfigSection().getLong("Duration"));
		this.angleIncrement = getConfigSection().getDouble("AngleIncrement");
