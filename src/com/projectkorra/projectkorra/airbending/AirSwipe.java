...
public class AirSwipe extends AirAbility {
	...
	public AirSwipe(final Player player, final boolean charging) {
		...
		this.arc = getConfigSection().getInt("Arc");
		this.arcIncrement = getConfigSection().getInt("StepSize");
		this.maxChargeTime = getConfigSection().getLong("MaxChargeTime");
		...
		this.speed = getConfigSection().getDouble("Speed") * (ProjectKorra.time_step / 1000.0);
		this.range = getConfigSection().getDouble("Range");
		this.radius = getConfigSection().getDouble("Radius");
		this.maxChargeFactor = getConfigSection().getDouble("ChargeFactor");
