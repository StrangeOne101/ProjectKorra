...
public class TempCommand extends PKCommand {
	...
	private final String updateSuccessOther;
	...
	private final String[] updateAliases = {"update", "u"};
	...
	public TempCommand() {
		super("temp", "/bending temp <Add/Extend/Reduce/Update/Remove> <Player> [Time]",
				ConfigManager.languageConfig.get().getString("Commands.Temp.Description.Main"), new String[] { "temp", "te", "temporary"});
		...
		this.updateSuccess = ChatUtil.color(ConfigManager.languageConfig.get().getString("Commands.Temp.Update.Success"));
		this.updateSuccessOther = ChatUtil.color(ConfigManager.languageConfig.get().getString("Commands.Temp.Update.SuccessOther"));
	...
	public void execute(final CommandSender sender, final List<String> args) {
		if (!this.correctLength(sender, args.size(), 1, 4)) {
			...
			if (Arrays.asList(addAliases).contains(args.get(0).toLowerCase())) {
				...
				help(sender, "Reduce");
			} else if (Arrays.asList(updateAliases).contains(args.get(0).toLowerCase())) {
				help(sender, "Update");
		...
		if (Arrays.asList(addAliases).contains(args.get(0).toLowerCase())) {
			...
			if (!hasPermission(sender, "update")) return;
			BendingPlayer.getOrLoadOfflineAsync(player).thenAccept(bPlayer -> {
				updateElement(element, bPlayer, sender, time);
			});
		}
	}
	...
	private void addElement(Element element, OfflineBendingPlayer bPlayer, CommandSender sender, long time) {
		...
		String newExpiryString = TimeUtil.formatTime(time);
	...
	private void extendElement(Element element, OfflineBendingPlayer bPlayer, CommandSender sender, long time) {
		...
		long newExpiry = add ? time + System.currentTimeMillis() : bPlayer.getTempElements().get(element) + time;
	...
	private void updateElement(Element element, OfflineBendingPlayer bPlayer, CommandSender sender, long time) {
		//If they don't have it, or they do but it has already expired
		boolean add = !bPlayer.getTempElements().containsKey(element) || bPlayer.getTempElements().get(element) < System.currentTimeMillis();

		long newExpiry = time + System.currentTimeMillis();
		String newExpiryString = TimeUtil.formatTime(time);

		String message = add ? (element == Element.AVATAR ? this.addedSuccessAvatar : this.addedSuccess) : this.updateSuccess;
		String messageOther = add ? (element == Element.AVATAR ? this.addedSuccessOtherAvatar : this.addedSuccessOther) : this.updateSuccessOther;
	...
	private boolean removeElement(Element element, OfflineBendingPlayer bPlayer, CommandSender sender) {
		//If they don't have it, or they do but it has already expired
		if (!bPlayer.getTempElements().containsKey(element) || bPlayer.getTempElements().get(element) < System.currentTimeMillis()) { //or it has already expired
			ChatUtil.sendBrandingMessage(sender, ChatColor.RED + this.removeElementNotFound
					.replace("{element}", element.getColor() + element.getName())
	...
	protected List<String> getTabCompletion(final CommandSender sender, final List<String> args) {
		...
		if (args.size() == 0) {
			for (String cmd : new String[]{"add", "extend", "reduce", "update", "remove"}) {
